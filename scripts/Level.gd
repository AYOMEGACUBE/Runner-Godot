...
func _spawn_step_in_active_corridor(target_top_y: float) -> bool:
	...
	if DEBUG_LOG:
		_log("[SPAWN_STEP] difficulty=%.4f target_top_y=%.1f last_main_pos=%s" % [difficulty, target_top_y, last_main_pos])
	...
	if DEBUG_LOG:
		_log("[SPAWN_STEP] seg_main=%d next_y=%.1f reach=%.1f jump=%.1f grav=%.1f speed=%.1f" % [seg_main, next_y, reach, p_jump, p_grav, p_speed])
