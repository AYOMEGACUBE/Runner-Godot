...
func _ready() -> void:
	...
	_log("[PLATFORM_READY] pos=%s size=%s is_crumbling=%s coin_chance=%.2f" % [global_position, size, is_crumbling, coin_spawn_chance])
	...
	if coin_scene != null and coin_spawn_chance > 0.0:
		...
		if roll < coin_spawn_chance:
			_log("[PLATFORM_READY] spawning coin roll=%.3f" % roll)
			await get_tree().create_timer(0.1).timeout
			_spawn_coin_above()
		else:
			_log("[PLATFORM_READY] no coin roll=%.3f" % roll)
